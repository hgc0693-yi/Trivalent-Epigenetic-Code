...
$infile = "/home/yi-maru/RNA-seq/research/DEG_and_epigenomefactors_relation/only_genebody/5hmC_5mC_H3K4me1_H4K8ac/heatmap/preparing_combined_data/combined_whole_factors.txt";
...
sudo <- 0.0001
GB_5hmC <- log2( (data[,3]+sudo) / (data[,7]+sudo) ) # ES / Fib
head(GB_5hmC)

GB_5mC <- log2( (data[,4]+sudo) / (data[,8]+sudo) ) # ES / Fib
head(GB_5mC)

GB_H3K4me1 <- log2( (data[,5]+sudo) / (data[,9]+sudo) ) # ES / Fib
head(GB_H3K4me1)

GB_H4K8ac <- log2( (data[,6]+sudo) / (data[,10]+sudo) ) # ES / Fib
head(GB_H4K8ac)


gene_name <- rownames(data)
all_data <- cbind(GB_5hmC, GB_5mC, GB_H3K4me1, GB_H4K8ac)
